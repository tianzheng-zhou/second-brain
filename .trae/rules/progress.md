...
| 2026-02-26 | **UI** | **话题文件夹管理** | 1. **Backend**: 新增 `folders` 表及 API (`/folders`, `/conversations/{id}/folder`)。<br>2. **Frontend**: 通过 `custom.js` 在侧边栏 "New Chat" 按钮旁注入 "New Folder" 按钮，支持创建文件夹。 |

## 待办事项 (Backlog)
- [ ] **文件夹视图**: 在侧边栏展示文件夹列表，并支持按文件夹过滤会话 (需深入定制 Chainlit 前端或使用 iframe)。
